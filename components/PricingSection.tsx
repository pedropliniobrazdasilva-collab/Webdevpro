...
const pricingPlans = [
  {
    name: 'Plano Básico',
    price: '150',
    description: 'Ideal para quem precisa de uma presença online rápida e eficaz.',
    features: [
  ...
  },
  {
    ...
    price: '325',
    description: 'A solução perfeita para empresas que buscam crescer e captar mais clientes.',
    features: [
  ...
  },
  {
    name: 'Plano Avançado',
    price: '575',
    description: 'Para negócios que exigem uma plataforma completa e otimizada para máxima conversão.',
    features: [
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-stretch">
          {pricingPlans.map((plan, index) => (
            <div
              key={index}
              className={`relative flex flex-col bg-slate-900 rounded-lg shadow-xl p-8 transition-all duration-300 group ${plan.highlight ? 'border-2 border-primary-500 shadow-primary-500/20' : 'border-2 border-slate-800 hover:border-primary-500/50'}`}
            >
              {plan.highlight && (
                 <>
              ...
              <a
                href="#contato"
                className={`w-full text-center py-3 px-6 rounded-lg font-semibold transition-all duration-300 ease-in-out transform hover:-translate-y-1 ${plan.highlight ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white hover:shadow-lg hover:shadow-primary-500/30' : 'bg-slate-700 text-white hover:bg-primary-500'}`}
              >
                Solicitar Orçamento
              </a>
            </div>
